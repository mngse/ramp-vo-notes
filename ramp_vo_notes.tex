...
The paper describes RAMP-VO as an \emph{end-to-end learned} image-and-event VO system and states that ``we train RAMP-VO'' on an event-augmented version of TartanAir while following the DPVO training scheme and hyperparameters. Concretely, the training set is built from TartanAir image sequences augmented with synthetic events generated by VID2E; during training, frame selection/filtering follows DPVO, and the model is trained for \(350{,}000\) steps on sequences of \(15\) images and \(30\) event stacks.

For the architecture itself, the trainable neural components are the RAMP encoder blocks \(E_k^s,G_k^s,H_k\), the multi-scale fusion modules producing \(\mathbf{m}_j,\mathbf{c}_j\), and the DPVO backend components that predict \(\Delta_{li}\) and \(\sigma_{li}\). The paper does not state that the DPVO backend is frozen and then reused as-is. Since the system is presented as end-to-end learned and the differentiable BA layer is part of the trainable pipeline, the natural reading is that the neural parts of the DPVO backend are retrained jointly with the new RAMP encoder rather than kept fixed. This last point is an inference from the paper's description, not an explicit sentence in the paper.
